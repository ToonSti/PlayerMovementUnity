...
public class CameraController : MonoBehaviour
{
    ...
    [SerializeField] private float camHeight = 1.5f;        // height the camera is above the player
    ...
    void Update()
    {
        ...
        // run the MoveCamera function and send it the just defined currentCamDistance
        MoveCamera(currentCamDistance);
    }

    // function called everytime mentioned
    void MoveCamera(float currentCamDistance)
    {
        // the position that the camera should be from the player
        camDirection = new Vector3(0, camHeight, -currentCamDistance);
